    ...
    <div className="min-h-dvh bg-bg-card pb-32">
      <header className="pt-6 pb-4 bg-white sticky top-0 z-30 border-b border-gray-100">
        <div className="max-w-5xl mx-auto px-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl text-[var(--color-charcoal)]">Saved learnings</h1>
            <p className="text-sm text-text-muted">Your reflections from check-ins.</p>
          </div>
        </div>
      </header>

      <div className="px-6 pt-6 max-w-5xl mx-auto">
        {learningCheckIns.length === 0 ? (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 text-center">
            <Sparkles className="w-10 h-10 text-brand-primary mx-auto mb-4" />
            <p className="text-lg font-semibold text-[var(--color-charcoal)]">No saved learnings yet</p>
            <p className="text-sm text-text-muted mt-2">
              Tap the heart icon on a weekly check-in to save what you learned.
            </p>
            <button
              onClick={() => router.push("/progress")}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-2 text-base text-[var(--color-charcoal)] leading-relaxed">
                      {checkIn.reflection || "No text provided."}
                    </p>
                  </div>
                  <button
